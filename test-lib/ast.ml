...
let rec string_of_expression = function
    ...
    | Sub(e1, e2 ) -> (string_of_expression e1) ^ "-" ^ 
                      (string_of_expression e2)
    | Mult(e1, e2) -> (string_of_expression e1) ^ "*" ^ 
                      (string_of_expression e2)
    | Div(e1, e2) -> (string_of_expression e1) ^ "/" ^ 
                     (string_of_expression e2)
    | Expo(e1, e2) -> (string_of_expression e1) ^ "^" ^ 
                      (string_of_expression e2)
    | Mod(e1, e2) -> (string_of_expression e1) ^ "%" ^ 
                     (string_of_expression e2)
    | FMult(f1, f2) -> (string_of_expression f1) ^ "*" ^ 
                       (string_of_expression f1)
    | FAdd(f1, f2) -> (string_of_expression f1) ^ "+" ^ 
                      (string_of_expression f2)
    | FSub(f1, f2) -> (string_of_expression f1) ^ "-" ^ 
                      (string_of_expression f2)
    | FDiv(f1, f2) -> (string_of_expression f1) ^ "/" ^ 
                      (string_of_expression f2)
    | FuncCall( s, e ) -> "function call " ^ s
    | Assign( s, e ) -> "assign " ^ s ^ " = " ^ (string_of_expression e)
    | And( b1, b2) -> (string_of_expression b1) ^ " && " ^ 
                      (string_of_expression b2)
    | Or( b1, b2) -> (string_of_expression b1) ^ " || " ^ 
                     (string_of_expression b2)
